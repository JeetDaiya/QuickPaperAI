...
class RedisOTPStore(OTPStore):
    ...
    async def save_otp(self, email : str, otp_code: str, purpose: str, expire_seconds: int = 600) -> None:
        try:
            ...
            raise RepositoryError(email=email, purpose=purpose) from e
    ...
    async def delete_otp(self, email: str, purpose: str) -> None:
        # Deliberately best-effort: this runs after the OTP has already been verified/used,
        # so a cleanup failure here shouldn't fail the caller's request. Log-and-continue, not
        # a leftover — don't convert to raise.
        try:
            ...
            print(f"Failed to delete otp: {email}: {purpose}, error: {e}")
