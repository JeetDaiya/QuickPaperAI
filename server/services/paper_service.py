...
from core.interfaces.db import PaperRepository, ChunkRepository
...
class PaperService:
    ...
    async def cancel_generation(self, thread_id: str, req: Request):
        try:
            ...
            try:
                self.cloud_storage.delete_file(file_path=f"{thread_id}/paper.pdf")
                self.cloud_storage.delete_file(file_path=f"{thread_id}/answer.pdf")
                self.cloud_storage.delete_file(file_path=f"{thread_id}/paper.docx")
            ...
            if self.local_storage.exists(file_path=f"{thread_id}/paper.pdf"):
                self.local_storage.delete_file(file_path=f"{thread_id}/paper.pdf")
