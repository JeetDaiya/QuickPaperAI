...
async def upload_completed_paper_to_storage(thread_id : str,
    ...
    try:
        ...
        filenames = ["paper.pdf", "answer.pdf", "paper.docx"]
        files_data = {}

        try:
            for filename in filenames:
                relative_path = f"{thread_id}/{filename}"
                files_data[filename] = local_storage.get_file(file_path=relative_path)
        ...
        for filename, file_bytes in files_data.items():
            ...
            content_type = "application/pdf" if filename.endswith(
        ...
        try:
            ...
            paper_repo.upload_paper_metadata(metadata=insert_data)
...
async def get_generation_status(thread_id: str, req: Request, current_user : dict = Depends(get_current_user), local_storage: StorageService = Depends(get_local_storage)):
    ...
    if not snapshot.next or not snapshot.tasks:
        if local_storage.exists(file_path=f"{thread_id}/paper.pdf"):
            return {
                "status": "completed",
                "files": {
...
async def download_file(thread_id: str, filename: str, preview: bool = False, current_user : dict = Depends(get_current_user), paper_repo : PaperRepository = Depends(get_paper_repository), cloud_storage : StorageService = Depends(get_cloud_storage), local_storage : StorageService = Depends(get_local_storage)):
    ...
    try:
        response = paper_repo.get_paper_metadata(thread_id=thread_id, paper_name=target_column)
...
async def save_to_cloud(thread_id: str, req: Request, current_user : dict = Depends(get_current_user), paper_repo: PaperRepository = Depends(get_paper_repository), cloud_storage : StorageService = Depends(get_cloud_storage), local_storage : StorageService = Depends(get_local_storage)):
    try:
        agent = req.app.state.agent
        config = {"configurable": {"thread_id": thread_id}}
        await upload_completed_paper_to_storage(thread_id=thread_id, agent=agent, config=config, user_id=str(current_user["id"]), paper_repo=paper_repo, local_storage=local_storage, cloud_storage=cloud_storage)
        return {"status": "success"}
    except Exception as e:
...
async def cancel_generation(thread_id: str, req: Request, current_user : dict = Depends(get_current_user), paper_repo : PaperRepository = Depends(get_paper_repository), cloud_storage : StorageService = Depends(get_cloud_storage), local_storage : StorageService = Depends(get_local_storage)):
    ...
    try:
        ...
        try:
            paper_repo.delete_paper_metadata(thread_id=thread_id)
        ...
        try:
            ...
            cloud_storage.delete_file(file_path=f"{thread_id}/paper.pdf")
            cloud_storage.delete_file(file_path=f"{thread_id}/answer.pdf")
            cloud_storage.delete_file(file_path=f"{thread_id}/paper.docx")
        ...
        if local_storage.exists(file_path=f"{thread_id}/paper.pdf"):
            local_storage.delete_file(file_path=f"{thread_id}/paper.pdf")
